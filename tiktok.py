...
print(pyfiglet.figlet_format("G e o . K I N G", font="slant"))
print( '''
          [!] thank you for help me :Gio King [!]            
⚠️WARNING : I AM NOT RESPONSIBLE FOR THE MISUSE OF THIS TOOL !
                  [😡]TIK TOK ID FLOWER[😡]
   *  Author  : AFGHANSTAN CRIMINAL HACK 
   *  GitHub  : Criminal313
   *  whatsapp: +93783361438
   *  YouTub  : ERROR
   *  FacebooK: MAFIA REYESS
   *  License : MIT         
...
def title1():  # Update the title IF option 1 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title Gio King V3 ^| Views Sent: {beautify(Views)} ^| Elapsed Time: {time_elapsed}')
...
def title2():  # Update the title IF option 2 was picked.
    ...
    while True:
        time_elapsed = strftime('%H:%M:%S', gmtime(time() - start))
        system(f'title  V3 ^| Hearts Sent: {beautify(Hearts)} ^| Elapsed Time: {time_elapsed}')
...
if auto == 1:
    ...
    print("[+] This program was created by @MMS. [github.com/IRTAZA]")
    print("[+] This program was origionally uploaded to github.com/MMS/tiktodv3.")
